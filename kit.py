...
/* copertă cu fișier real (schițele ilustrate): blurată până la cumpărare; clară, cu zoom și descărcare, după */
.cover .photo{position:absolute;inset:0;width:100%;height:100%;object-fit:cover;object-position:center top;transition:filter .5s ease,transform .5s ease;-webkit-user-drag:none;user-select:none}
.cover .photo-link{position:absolute;inset:0;display:block;z-index:1}
.cover.locked .photo{filter:blur(4px) saturate(.85) brightness(.8);transform:scale(1.12)}
.cover.owned .photo-link{cursor:zoom-in}
.card:hover .cover.owned .photo{transform:scale(1.04)}
.cover .lock{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);z-index:3;display:inline-flex;align-items:center;gap:.4rem;max-width:92%;background:rgba(16,24,40,.74);color:#fff;font-size:.72rem;font-weight:800;letter-spacing:.03em;line-height:1.25;padding:.42rem .8rem;border-radius:999px;border:1px solid rgba(255,255,255,.35);backdrop-filter:blur(4px);box-shadow:0 8px 24px rgba(16,24,40,.35);pointer-events:none;text-align:center}
.cover .lock svg{flex-shrink:0}
.cover .lock.lg{flex-direction:column;gap:.35rem;padding:1rem 1.4rem;border-radius:18px;font-size:.98rem;letter-spacing:0}
.cover .lock.lg small{display:block;font-weight:600;font-size:.78rem;opacity:.9}
.cover .owned-tag{position:absolute;top:.6rem;left:50%;transform:translateX(-50%);z-index:2;display:inline-flex;align-items:center;gap:.3rem;background:#059669;color:#fff;font-size:.68rem;font-weight:800;padding:.22rem .6rem;border-radius:999px;letter-spacing:.04em;white-space:nowrap;box-shadow:0 4px 12px rgba(5,150,105,.35)}
.cover:not(.owned) .owned-tag,.cover:not(.locked) .lock{display:none}
.unlock-note{display:flex;gap:.6rem;align-items:flex-start;background:#F4F1FF;border:1.5px solid #DDD6FE;border-radius:14px;padding:.75rem .9rem;font-size:.86rem;color:var(--ink-2);margin-top:.8rem;line-height:1.45}
.unlock-note svg{flex-shrink:0;color:var(--violet);margin-top:2px}
.unlock-note a{color:var(--violet);font-weight:700;text-decoration:underline}
.unlock-note.ok{background:#ECFDF5;border-color:#A7F3D0}
.unlock-note.ok svg{color:#059669}
.dl-row{display:flex;gap:.6rem;flex-wrap:wrap;margin-top:.8rem}
.dl-list{display:grid;gap:.5rem;margin-top:.8rem}
.dl-item{display:flex;align-items:center;justify-content:space-between;gap:.8rem;background:#F8F7FF;border:1.5px solid var(--line);border-radius:12px;padding:.6rem .8rem;font-size:.88rem;font-weight:600}
.dl-item .acts{display:flex;gap:.4rem;flex-shrink:0}
...
PRODUCTS = [
    ...
    dict(id=35, title="Schiță ilustrată: „O scrisoare pierdută” de I.L. Caragiale – analiză completă", cat="schite-si-conspecte", lang="Română", level="Liceu", fmt="PDF", pages="1 fișă A3", price=19, old=0, badge="Nou", rating=4.9, votes=12, icon="story", img="uploads/schita-o-scrisoare-pierduta.jpg", preview="uploads/preview/schita-o-scrisoare-pierduta.jpg"),
    dict(id=36, title="Schiță ilustrată: „Plumb” de George Bacovia – analiză completă", cat="schite-si-conspecte", lang="Română", level="Liceu", fmt="PDF", pages="1 fișă A3", price=19, old=0, badge="Nou", rating=4.8, votes=9, icon="book", img="uploads/schita-plumb.jpg", preview="uploads/preview/schita-plumb.jpg"),
...
def file_info(p):
    """Fișierul real al unui produs (`img`) + previzualizarea blurată (`preview`) afișată înainte de cumpărare.

    Fișierul complet nu este niciodată încărcat în pagină până când produsul nu apare într-o comandă
    (vezi `Caleido.applyLocks` în JS-ul comun). Returnează None pentru produsele fără fișier real.
    """
    img = p.get("img", "")
    if not img:
        return None
    preview = p.get("preview") or ("uploads/preview/" + img.rsplit("/", 1)[-1])
    return {"full": img, "preview": preview, "title": p["title"], "ext": img.rsplit(".", 1)[-1].upper()}


# id -> {full, preview, title, ext}; injectat în JS-ul comun (footer) ca `FILES`
FILES = {str(p["id"]): file_info(p) for p in PRODUCTS if p.get("img")}
...
(function(){
  ...
  function fileFor(id){ return FILES[String(id)]||null }

  /* Fișierele reale (schițele) rămân blurate — se încarcă doar previzualizarea — până când produsul
     apare într-o comandă. După cumpărare: imagine clară, zoom în mărime completă și buton de descărcare.
     Elementele cu data-owned-only / data-locked-only="ID" se afișează doar în starea respectivă. */
  function applyLocks(){
    var own=getOwned();
    document.querySelectorAll('.cover[data-id]').forEach(function(c){
      var id=String(c.dataset.id), f=FILES[id]; if(!f) return;
      var ok=own.indexOf(id)>-1, img=c.querySelector('.photo'), a=c.querySelector('.photo-link');
      c.classList.toggle('owned',ok); c.classList.toggle('locked',!ok);
      if(img){
        var src=ok?f.full:f.preview; if(img.getAttribute('src')!==src){ img.setAttribute('src',src) }
        var t=img.dataset.alt||f.title; img.alt=ok?t:'Previzualizare blurată: '+t;
      }
      if(a){
        if(ok){ a.href=f.full; a.target='_blank'; a.rel='noopener'; a.title='Deschide în mărime completă' }
        else { a.href=a.dataset.lockedHref||('produs.html?id='+id); a.removeAttribute('target'); a.removeAttribute('rel'); a.title='Se deblochează după cumpărare' }
      }
    });
    document.querySelectorAll('[data-owned-only]').forEach(function(el){ el.hidden=own.indexOf(String(el.dataset.ownedOnly))<0 });
    document.querySelectorAll('[data-locked-only]').forEach(function(el){ el.hidden=own.indexOf(String(el.dataset.lockedOnly))>-1 });
  }
  ...
  window.Caleido={getCart:getCart,setCart:setCart,addToCart:addToCart,getWish:getWish,toggleWish:toggleWish,toast:toast,paint:paint,
                  getOwned:getOwned,isOwned:isOwned,fileFor:fileFor,applyLocks:applyLocks};
...
def footer(extra_js=""):
    ...
    return (FOOTER.replace("__CATLINKS__", catlinks).replace("__UP__", up).replace("__FILES__", files)
            .replace("</body>", extra_js + "</body>"))
...
def product_card(p):
    ...
    # Produs cu fișier real: se randează BLURAT (doar previzualizarea), cu lacăt; JS-ul comun (Caleido.applyLocks)
    # îl deblochează — imagine clară, zoom și buton „Descarcă” în locul lui „Adaugă” — după ce apare într-o comandă.
    photo = (
        '<a class="photo-link" href="produs.html?id=%(id)d" title="Se deblochează după cumpărare">'
        '<img class="photo" src="%(preview)s" alt="Previzualizare blurată: %(title)s" data-alt="%(title)s" '
        'loading="lazy" decoding="async" width="200" height="300" draggable="false"></a>\n        '
        '<span class="lock">%(lock)s Se deblochează după cumpărare</span>'
        '<span class="owned-tag">%(check)s Achiziționat</span>'
        % dict(id=p["id"], preview=f["preview"], title=title_attr, lock=svg("lock", 13, 2.2), check=svg("check", 12, 3))
    ) if f else ""
    download = (
        '\n        <a class="add" href="%s" download data-owned-only="%d" hidden>%s Descarcă</a>'
        % (f["full"], p["id"], svg("download", 15, 2))
    ) if f else ""
    return """
    <article class="card reveal" data-id="%(id)d" data-cat="%(cat)s" data-lang="%(lang)s" data-level="%(level)s" data-fmt="%(fmt)s" data-price="%(price)d" data-votes="%(votes)s" data-rating="%(rating)s" data-title="%(title_lower)s">
      <div class="cover%(has_img)s"%(cover_data)s style="background:%(grad)s">
        %(photo)s%(badge)s
      ...
      <div class="card-foot">
        ...
        <button class="add" data-id="%(id)d" data-title="%(title_attr)s" data-price="%(price)d"%(locked_only)s>%(cart)s Adaugă</button>%(download)s
      </div>
    </article>""" % dict(
        ...
        badge=badge, old=old, icon=svg(p["icon"], 28, 1.7), cattitle=cat_title(p["cat"]), photo=photo, download=download,
        has_img=" has-img locked" if f else "", cover_data=' data-id="%d"' % p["id"] if f else "",
        locked_only=' data-locked-only="%d"' % p["id"] if f else "",
